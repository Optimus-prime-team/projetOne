...
def drop_collection() :
    mycol = pymongo.MongoClient()["mydatabase2"]["offres_indeed2"]
    mycol.drop()
...
def get_connection() :
    ...
    if "mydatabase2" not in dblist:
        print('db created')
      
    mydb = client["mydatabase2"]
    
    collist = mydb.list_collection_names()
    if "offres_indeed2" not in collist:
        print('collection created')
    mycol = mydb["offres_indeed2"]
    return mycol
...
def save_offers (liste_offre) :
    ...
    if len(df_to_add) != 0 :
        ...
        if force_d == True:
            print(colored('WARNING force to not insert in DB "Rien à ajouter !"', 'yellow', attrs=["bold"]))
        else:

...
search_in_toulouse =  df[df['city_querry'] == "Toulouse"]

# mise en place du titre
title ="Nombre d'offre scrappées : "+str(len(search_in_toulouse))


# données pour le pie % d'offre par type d'emploi
search_in_toulouse =  df[df['city_querry'] == "Toulouse"]
labels_job = list(search_in_toulouse.groupby('job_querry')['adId'].groups.keys())
nbr_job = search_in_toulouse.groupby('job_querry')['adId'].count().to_numpy().astype(int)
...
df_avec_salaire = search_in_toulouse[search_in_toulouse['salary'] > 0]
df_sans_salaire = search_in_toulouse[search_in_toulouse['salary'] == 0]

# print(avec_salaire)
# exit()
# df_avec_salaire = df[df['salary'].isnull()==False]

nbr_salaire = [len(df_avec_salaire), (len(search_in_toulouse) - len(df_avec_salaire))]
...
df_mean_salaire_par_cat = search_in_toulouse[search_in_toulouse['salary'] > 0].groupby('job_querry')

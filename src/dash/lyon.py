...
search_in_lyon =  df[df['city_querry'] == "Lyon"]

# mise en place du titre
title ="Nombre d'offre scrappées : "+str(len(search_in_lyon))


# données pour le pie % d'offre par type d'emploi
search_in_lyon =  df[df['city_querry'] == "Lyon"]
labels_job = list(search_in_lyon.groupby('job_querry')['adId'].groups.keys())
nbr_job = search_in_lyon.groupby('job_querry')['adId'].count().to_numpy().astype(int)
...
df_avec_salaire = search_in_lyon[search_in_lyon['salary'] > 0]
df_sans_salaire = search_in_lyon[search_in_lyon['salary'] == 0]

# print(avec_salaire)
# exit()
# df_avec_salaire = df[df['salary'].isnull()==False]

nbr_salaire = [len(df_avec_salaire), (len(search_in_lyon) - len(df_avec_salaire))]
...
df_mean_salaire_par_cat = search_in_lyon[search_in_lyon['salary'] > 0].groupby('job_querry')

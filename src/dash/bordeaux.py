...
search_in_bordeaux =  df[df['city_querry'] == "Bordeaux"]

# mise en place du titre
title ="Nombre d'offre scrappées : "+str(len(search_in_bordeaux))


# données pour le pie % d'offre par type d'emploi
search_in_bordeaux =  df[df['city_querry'] == "Bordeaux"]
labels_job = list(search_in_bordeaux.groupby('job_querry')['adId'].groups.keys())
nbr_job = search_in_bordeaux.groupby('job_querry')['adId'].count().to_numpy().astype(int)
...
df_avec_salaire = search_in_bordeaux[search_in_bordeaux['salary'] > 0]
df_sans_salaire = search_in_bordeaux[search_in_bordeaux['salary'] == 0]

# print(avec_salaire)
# exit()
# df_avec_salaire = df[df['salary'].isnull()==False]

nbr_salaire = [len(df_avec_salaire), (len(search_in_bordeaux) - len(df_avec_salaire))]
...
df_mean_salaire_par_cat = search_in_bordeaux[search_in_bordeaux['salary'] > 0].groupby('job_querry')
